...
<div class="wrap">
    ...
    <!-- Mail-Debug-Log -->
    <div class="card" style="margin-top: 2rem;">
        <h2>📧 Mail-Log <span style="font-size:13px; font-weight:400; color:#646970;">(letzte 100 Versandvorgänge)</span></h2>
        <?php
        $mail_log = get_option('dp_mail_debug_log', array());
        if (!is_array($mail_log)) $mail_log = array();
        ?>
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:1rem;">
            <span style="color:#646970; font-size:13px;"><?php echo count($mail_log); ?> Eintr<?php echo count($mail_log) !== 1 ? 'äge' : 'ag'; ?></span>
            <?php if (!empty($mail_log)): ?>
            <button id="dp-clear-mail-log" class="button button-secondary" style="color:#b32d2e; border-color:#b32d2e;">
        ...
        <?php if (empty($mail_log)): ?>
            <p style="color:#646970; font-style:italic;">Noch keine Mails geloggt. Mails werden ab sofort aufgezeichnet.</p>
        <?php else: ?>
                ...
                <tr>
                    ...
                    <td style="text-align:center;">
                        <?php if (($entry['status'] ?? '') === 'ok'): ?>
                            <span style="color:#016d1c; font-weight:600;">✓ OK</span>
                        <?php else: ?>
                            <span style="color:#b32d2e; font-weight:600;">✗ Fehler</span>
                        <?php endif; ?>
                    </td>
                    <td style="color:#b32d2e; font-size:12px;"><?php echo esc_html($entry['error'] ?? ''); ?></td>
                </tr>
            <?php endforeach; ?>
            </tbody>
        </table>
    ...
    <script>
    jQuery(function($) {
        $('#dp-clear-mail-log').on('click', function() {
            if (!confirm('Mail-Log wirklich leeren?')) return;
            var btn = $(this);
            btn.prop('disabled', true).text('Wird geleert…');
            $.post(ajaxurl, {
                action: 'dp_clear_mail_log',
                nonce:  '<?php echo esc_js(wp_create_nonce('dp_ajax_nonce')); ?>'
            }, function(resp) {
                if (resp.success) {
                    location.reload();
                } else {
                    alert('Fehler: ' + (resp.data && resp.data.message ? resp.data.message : 'Unbekannt'));
                    btn.prop('disabled', false).text('Log leeren');
                }
            });
        });
    });
